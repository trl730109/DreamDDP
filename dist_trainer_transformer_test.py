...
def transformer_ssgd(optimizer_name, dnn, dataset, data_dir, nworkers, lr, batch_size, nsteps_update, max_epochs, nwpernode, pretrain, num_steps, compressor, density, strategy, threshold, gradient_path=None, momentum_correction=False, prefix=None, lr_decay=None,
                         nsteps_param_diversity=None, check_param_diversity=None, args=None, profile=False, profiler_trace=False):
    rank = dist.get_rank()
    ...
    # 量化模型（8bit/4bit）在各 rank 上已经通过 from_pretrained 加载了相同权重，
    # 且 state_dict 中权重位于 CPU，当前进程组 backend 为 NCCL（仅支持 CUDA 张量），
    # 此时再对 CPU tensor 做 broadcast 会触发 "No backend type associated with device type cpu"。
    # 因此在量化场景下直接跳过参数 broadcast。
    if args.load_quantization in ("8bit", "4bit"):
        log_info(f"Skip parameter broadcast for quantized model with load_quantization={args.load_quantization}.")
    else:
    ...
    if profiler_trace:
        ...
        profiler_ctx = torch.profiler.profile(
            ...
            schedule=torch.profiler.schedule(wait=0, warmup=10, active=20, repeat=1),
            on_trace_ready=torch.profiler.tensorboard_trace_handler(prof_log_dir),
            record_shapes=True,
            with_stack=False,
        )
        log_info('Profiler trace enabled: %s (rank %d)', prof_log_dir, rank)
    else:
        profiler_ctx = __import__('contextlib').nullcontext(_DummyProfSgd())

    
    if args.cpu_clock:
        torch.cuda.synchronize()
        cpu_clock_start = time.time()
    ...
    with profiler_ctx as prof:
        for epoch in range(max_epochs):
            ...
            for i in range(iters_per_epoch//nsteps_update):
                ...
                if i % display == 0 and i > 0 and not profiler_trace and not args.cpu_clock: 
    ...
    if args.cpu_clock:
        ...
        with open(clock_path, 'w') as f:
            f.write(f'cpu_clock_start: {cpu_clock_start}\n')
            f.write(f'cpu_clock_end: {cpu_clock_end}\n')
            f.write(f'elapsed: {elapsed}\n')
        log_info('CPU clock time saved to %s', clock_path)
        
        

    if profiler_trace:
        table_str = prof.key_averages().table(sort_by="cpu_time_total")
        table_path = os.path.join(prof_log_dir, f'key_averages_cpu_time_rank{rank}.txt')
...
def transformer_pipe_sgd(optimizer_name, overlap_scalar, dnn, dataset, data_dir, nworkers, lr, batch_size, nsteps_update, max_epochs, nwpernode, pretrain, num_steps, compressor, density, strategy, threshold, gradient_path=None, momentum_correction=False, prefix=None, lr_decay=None,
                         nsteps_param_diversity=None, check_param_diversity=None, args=None, profiler_trace=False):
    rank = dist.get_rank()
    log_info('the rank of current process: %d', rank)
    #print("The ssgd_with_horovod is called by rank: ", rank)
    ...
    if profiler_trace:
        ...
        profiler_ctx = __import__('contextlib').nullcontext(_DummyProfPipe())

    if args and args.cpu_clock:
        torch.cuda.synchronize()
        cpu_clock_start = time.time()
...
def transformer_localsgd(dnn, dataset, data_dir, nworkers, lr, batch_size, max_epochs, nwpernode, nsteps_update, tokenizer_name=None, nsteps_localsgd=20, lr_decay = 'step',
             check_param_diversity=None, nsteps_param_diversity=None, args=None, profile=False, profiler_trace=False):
    ...
    if profiler_trace:
        ...
        profiler_ctx = __import__('contextlib').nullcontext(_DummyProf())
        ...
        for epoch in range(max_epochs):
            ...
            for j in range(iters_per_epoch):
                ...
                if global_iters % nsteps_localsgd == 0:
                    log_info(f' Iter {global_iters} start comm...')
                    if not profiler_trace and not args.cpu_clock:
                    ...
                    if args.finetune_type == "lora":
                        ...
                        for name, param in trainer.net.named_parameters():
                            # 极其关键：只筛选出被 LoRA 激活的、需要更新的参数
                            if param.requires_grad:
                                # 解析 module_name 供 Profile 统计使用
                                if "." in name:
                                    module_name = ".".join(name.split(".")[:-1])
                                else:
                                    module_name = name
                        ...
                        for layer_index, (name, module) in enumerate(trainer.net.named_modules()):
                            if len(list(module.children())) == 0:  
                                ...
                                pass
                    if not profiler_trace and not args.cpu_clock:
                        comm_time_acc += (time.time() - comm_start)
                        
                    
                if not profiler_trace and not args.cpu_clock:
                    iter_end = time.time()
                    train_time_acc += (iter_end - iter_start)
                    iter_time_acc += (iter_end - iter_start)
                    ExpTool.record(result_dict)
                    ExpTool.record({"global_iters": global_iters, "epochs": epoch, "train_loss": train_loss,
                                  "train_ppl": train_ppl})
                    record_param_diversity_with_period(trainer.net, global_iters, nsteps_param_diversity, check_param_diversity)
...
def transformer_full_pipe_localsgd(dnn, dataset, data_dir, nworkers, lr, batch_size, max_epochs, nwpernode, nsteps_update, tokenizer_name=None, nsteps_localsgd=20, lr_decay = 'step',
    ...
    # === 优化 1: 使用 List 替代 Dict，极大提升 append 和 wait 速度 ===
    _handle_list = []
    ...
    for layer_index, (name, module) in enumerate(trainer.net.named_modules()):
        if len(list(module.children())) == 0: 
            ...
            if len(valid_params) > 0:
                ...
                trigger_param._pipe_schedule_info = (args.group_num, group_index)
    ...
    for epoch in range(max_epochs):
        ...
        for i in range(iters_per_epoch//nsteps_update):
            ...
            for param in _registered_params:
                gap, begin_comm_iter = param._pipe_schedule_info
                param._sync_flag_ref[0] = (param.sgd_iters % gap) == begin_comm_iter
            ...
            ExpTool.record(result_dict)
            ExpTool.record({
                "global_iters": global_iters, "epochs": epoch, "train_loss": train_loss,
                "train_ppl": train_ppl, "total wait time": wait_time_acc,
                "total train time": train_time_acc, "total comm time": wait_time_acc,
                "total BP time": trainer.backward_acc,
                "total FP time": trainer.forward_acc,
...
def transformer_dream_ddp_optimized(dnn, dataset, data_dir, nworkers, lr, batch_size, max_epochs, nwpernode, nsteps_update, tokenizer_name=None, nsteps_localsgd=20, lr_decay = 'step',
             check_param_diversity=None, nsteps_param_diversity=None, args=None, profiler_trace=False):
    assert nsteps_localsgd > 1
    rank = dist.get_rank()
    log_info('the rank of current process: %d', rank)
    ...
    # Torch profiler trace for timing analysis
    class _DummyProf:
        def step(self): pass
        def __enter__(self): return self
        def __exit__(self, *args): pass
    ...
    with profiler_ctx as prof:
        for epoch in range(max_epochs):
            ...
            for i in range(iters_per_epoch // nsteps_update):
                ...
                if global_iters % 20 == 0:
                    log_info(f"global_iters: {global_iters}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cpu_clock', type=str, default='False', help='Enable CPU clock')
    ...
    parser.add_argument('--profiler_trace', type=str, default="False", help='Enable torch profiler trace (localsgd/ssgd/dream_ddp_optimized_traced), export chrome trace json')
    ...
    if (args.alg == 'transformer_localsgd'):
        ...
        transformer_localsgd(args.dnn, args.dataset, args.data_dir, args.nworkers, args.lr, args.batch_size, args.max_epochs, args.nwpernode, args.nsteps_update, tokenizer_name=None, nsteps_localsgd=args.nsteps_localsgd, lr_decay=args.lr_decay, 
             check_param_diversity=args.check_param_diversity, nsteps_param_diversity=args.nsteps_param_diversity, args=args, profile=args.profile, profiler_trace=args.profiler_trace)
