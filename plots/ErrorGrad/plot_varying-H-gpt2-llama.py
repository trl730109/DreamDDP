...
def run_convergence_vs_worker_modeltype_fixed_noise(worker, model, dataset="cifar10"):
    ...
    simple_name = f"{model}-{dataset}-{worker}workers-convergence_vH"
    markers = [None] * 7
    linestyles = [None] * 7
    # markers = ['o']*2 + ['v']*2 + ['D']*2
    # linestyles = ["-"] * 2 + ["--"] * 2 + [":"] * 3
    ...
    for alias in all_figures[CIFAR10_RES]:
        ...
        print(f"{alias} len x   {len(x)} len y  {len(y)}")
