...
#OUTPUT_PATH = '/home/shshi/gpuhome/tpds-logs'
OUTPUT_PATH = '/media/sf_Shared_Data/tmp/infocom20'
INPUT_PATH = '/home/shshi/gpuhome/tpds-logs'
FONTSIZE=16
...
def read_times_from_nccl_log(logfile, mode='allreduce'):
    print('fn: ', logfile)
    f = open(logfile)
    sizes = []
    times = []
...
def plot_p2platency(nccl=True):
    ...
    nworkers=16
    if nccl:
        filename = '/home/shshi/work/p2p-dl/logs/bigm-%s-nccl-mgd-%d.log' % (mode, nworkers)
        #filename = '/home/shshi/work/p2p-dl/logs/%s-nccl-mgd-%d.log' % (mode, nworkers)
    else:
    ...
    ax.plot(sizes, comms, label='Measured', linewidth=0.5)
    ax.plot(sizes, alpha+np.array(sizes)*beta, label=r'Predicted ($\alpha$=%.1e, $\beta$=%.1e)'%(Decimal(alpha), Decimal(beta)), linewidth=1)
    #ax.grid(linestyle=':')
        
    plt.xlabel('Size of parameters [bytes]')
    plt.ylabel(r'AllGather time [s]')
    plt.ylim(bottom=0, top=plt.ylim()[1]*1.2)
    plt.legend(ncol=1, loc=2, prop={'size': FONTSIZE})
    update_fontsize(ax, fontsize=FONTSIZE)
    #plt.subplots_adjust(left=0.16, bottom=0.17, top=0.98, right=0.98)
    plt.ticklabel_format(axis='x', style='sci', scilimits=(0,0))
    #plt.savefig('%s/%s.pdf' % (OUTPUT_PATH, 'allgathercommtime-%d'%nworkers), bbox_inches='tight')
    plt.show()
