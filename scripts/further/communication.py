...
def predict(filename, n, color, marker, label, sizes=None, ax=None, nccl=False):
    ...
    if sizes is None:
        if not nccl:
            ...
            sizes, comms, errors = read_times_from_nccl_log(filename)
            label='%d GPUs' % (n)
...
def plot_p2platency(nccl=True):
    ...
    if nccl:
        #filename = '%s/nccl/ring-allreduce-k80-%d.log' % (INPUT_PATH, 4)
        filename = '/home/shshi/work/p2p-dl/logs/allreduce-nccl-mgd-%d.log' % (16)
    else:
    ...
    ax.errorbar(sizes, comms, errors, label='Measured AllReduce Communication', linewidth=0.5)
    #ax.plot(sizes, comms, label='Measured AllReduce Communication', linewidth=0.5)
    ax.plot(sizes, alpha+np.array(sizes)*beta, label='Predicted (a=%e, b=%e)'%(Decimal(alpha), Decimal(beta)), linewidth=1)
    #ax.grid(linestyle=':')
        
    plt.xlabel('Size of parameters [bytes]')
    plt.ylabel(r'Communication time [s]')
    #plt.ylim(bottom=0, top=plt.ylim()[1]*1.2)
    plt.legend(ncol=1, loc=2, prop={'size': 10})
