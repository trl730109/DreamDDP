...
OUTPUTPATH='/media/sf_Shared_Data/tmp/further'
...
def plot_group_convergences():
    ...
    networks = ['alexnet', 'resnet50']#, 'lstm']#, 'lstman4']
    #networks = ['vgg16', 'resnet20', 'alexnet', 'resnet50', 'lstm', 'lstman4']
    num_rows = NFIGURES/NFPERROW
    for i, network in enumerate(networks):
        ax_row = i / NFPERROW
...
def plot_convergence(lax=None, network=None, subfig=None):
    ...
    if network == 'lstm':
        ...
        density=0.0025
        LOGHOME='/home/shshi/work/nvsync/nvlogs'
        line = plot_with_params(network, 16, 32, lr, 'hsw224', r'gTop-$k$ S-SGD',isacc=isacc,  prefix='allreduce-comp-gtopk-baseline-gwarmup-dc1-model-ijcai-wu2norm', nsupdate=1, density=density, force_legend=True, sg=1.50, force_color='b')
        plts.append(line)
    elif network == 'alexnet':
...
def read_speed(logfile):
    ...
    for line in f.readlines():
        ...
        if line.find('INFO Number of groups:') > 0:
            num_of_layers = int(line.split('INFO Number of groups:')[-1][:-1])
...
def read_multiple_speeds():
    ...
    dnn='inceptionv4';density=0.001;bs=32;lr=0.01;nw=16
    #dnn='alexnet';density=0.001;bs=256;lr=0.01;nw=16

    #dnn='inceptionv4';density=0.001;bs=64;lr=0.01;nw=4
    #dnn='vgg16i';density=0.001;bs=64;lr=0.01;nw=4
    #dnn='alexnet';density=0.001;bs=256;lr=0.01;nw=4
    #dnn='googlenet';density=0.001;bs=64;lr=0.01;nw=4
    #dnn='resnet50';density=0.001;bs=32;lr=0.01;nw=4
    #thresholds= [ 524288000, 16777216, 8388608, 1028576, 8192, 1024 ][::-1]
    #thresholds= [ 524288000, 16777216, 8388608, 1028576, 8192][::-1]
    thresholds= [ 524288000, 8192][::-1]
    print('[Dense] io, forward, backward, sparse, communication, total')
    speeds = []
    for thres in thresholds:
        #prefix='allreduce-gwarmup-dc1-model-infocom-thestest-thres-%dkbytes' % (thres/1024)
        #prefix='allreduce-gwarmup-dc1-model-infocom-debug-adamerge-thres-0kbytes'
        prefix='allreduce-comp-topk-gwarmup-dc1-model-infocom20-thres-%dkbytes' % (thres/1024)
