...
class Bar:
    initialized = False
    def __init__(self, start_time, duration, max_time, ax, type='p', index=1, is_optimal=False, start=0.3, title='Group1'):
        """
        ...
        if not Bar.initialized:
            ...
            self.ax_.set_ylim(bottom=bottom, top=0.7)
            self.ax_.get_yaxis().set_ticks([])
            self.ax_.xaxis.set_ticks_position('bottom')
            self.ax_.set_xticks([i*0.1 for i in range(0,11)])
            self.ax_.set_xticklabels(['%.3f'%(self.max_time_*(i*0.1)) for i in range(0,11)])
            #self.ax_.arrow(0, 0.00, 1.01, 0., fc='k', ec='k', lw=0.1, color='black', length_includes_head= True, clip_on = False, overhang=0, width=0.0004)
            self.ax_.annotate(r'$t$ $(s)$', (1.015, 0.07), color='black',  
        ...
        self.ax_.text(left_margin, self.start_+self.height_/2, title,horizontalalignment='right', color='black', va='center',size=fontsize)
