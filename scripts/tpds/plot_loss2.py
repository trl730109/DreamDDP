...
OUTPUTPATH='/home/shshi/gpuhome/plots/tpds'
...
def plot_convergence(lax=None, network=None, subfig=None):
    ...
    if network == 'lstm':
        ...
        global_max_epochs=120;bs=64;lr=0.01
        density=1.0
        line = plot_with_params(network, 16, bs, lr, 'hsw224', r'Dense-SGD', isacc=isacc, prefix='allreduce-gwarmup-dc1-model-tpds-convergence-ada-thres-0kbytes', nsupdate=1, density=density, force_legend=True, force_color='r')
        plts.append(line)
...
def read_multiple_speeds():
    ...
    LOGHOME='./logs';host='hsw218'; host='hsw224'; #host='hsw221'
    fp16 = True
    connection='10GbE-v2-r1'
    #connection='56GbIB-v2-r3'
    # NVIDIA
    #dnn='resnet50';density=0.001;bs=128;lr=0.8;nw=16
    #dnn='googlenet';density=0.001;bs=128;lr=0.01;nw=8
    #dnn='vgg16i';density=0.001;bs=128;lr=0.01;nw=8
    #dnn='resnet50';density=0.001;bs=128;lr=0.8;nw=16
    #dnn='resnet152';density=0.001;bs=128;lr=0.8;nw=16
    #dnn='densenet161';density=0.001;bs=64;lr=0.8;nw=16
    #dnn='densenet201';density=0.001;bs=64;lr=0.8;nw=16
    dnn='inceptionv4';density=0.001;bs=128;lr=0.8;nw=16
    #dnn='alexnet';density=0.001;bs=128;lr=0.01;nw=8
    #thresholds= [ 524288000, 16777216, 8388608, 1028576, 8192, 1024, 0 ][::-1]; ada=False
    #thresholds= [ 524288000, 16777216, 8388608, 1028576, 8192, 0][::-1]
    ...
    for thres in thresholds:
        ...
        if nw == 1:
            density=None;prefix='singlegpu-gwarmup-dc1-model-tpds10GbE'
            if fp16:
                density=None;prefix='singlegpu-gwarmup-dc1-model-tpds10GbE-fp16'
        else:
            if ada:
                #density=0.001;prefix='allreduce-comp-topk-gwarmup-dc1-model-infocom20-thestest-adamerge-thres-0kbytes'
                density=1.0;prefix='allreduce-gwarmup-dc1-model-tpds%s-ada-thres-0kbytes' % connection
                if fp16:
                    prefix='allreduce-gwarmup-dc1-model-tpds%s-fp16-ada-thres-0kbytes' % connection
            else:
                #density=0.001;prefix='allreduce-comp-topk-gwarmup-dc1-model-infocom20-thestest-thres-%dkbytes' % (thres/1024)
                density=1.0;prefix='allreduce-gwarmup-dc1-model-tpds%s-thres-%dkbytes' % (connection, thres/1024)
                if fp16:
                    prefix='allreduce-gwarmup-dc1-model-tpds%s-fp16-thres-%dkbytes' % (connection, thres/1024)
...
def plot_breakdowns_with_network(ax, network):
    ...
    avg_speed, avg_computation, avg_sparse,_,_,_ = read_speed(dense_logfile)
