...
#OUTPUTPATH='/media/sf_Shared_Data/tmp/nips2019/rebuttal'
OUTPUTPATH='/home/shshi/aaai2020'
...
if PLOT_TYPE == 'NORM':
    ...
    if NFIGURES ==3:
        ...
        FIGSIZE=(3.8*NFPERROW,2.4)
    else:
        FIGSIZE=(3.8*NFPERROW,3.4*NFIGURES/NFPERROW)
...
STANDARD_TITLES = {
        'resnet20': 'ResNet-20 on Cifar-10',
        'vgg16': 'VGG-16 on Cifar-10',
        'alexnet': 'AlexNet',
        'resnet50': 'ResNet-50 on ImageNet',
        'inceptionv4': 'Inception-v4',
        'lstmptb': 'LSTM-PTB on PTB',
        'lstm': 'LSTM-PTB on PTB',
        'lstman4': 'LSTM-AN4'
        }
...
def read_losses_from_log(logfile, isacc=False):
    ...
    return losses[0:max_epochs], times[0:max_epochs], average_delays[0:max_epochs], lrs[0:max_epochs]
...
def plot_group_convergences():
    ...
    #networks = ['resnet20', 'vgg16', 'resnet50', 'lstm']#, 'lstm']#, 'lstman4']
    networks = ['resnet20', 'inceptionv4', 'resnet50', 'lstm']#, 'lstm']#, 'lstman4']
    #networks = ['resnet50']#, 'lstm']#, 'lstman4']
    #networks = ['vgg16', 'resnet20', 'alexnet', 'resnet50', 'lstm', 'lstman4']
    num_rows = NFIGURES/NFPERROW
    for i, network in enumerate(networks):
        ax_row = i / NFPERROW
        ax_col = i % NFPERROW
        if NFIGURES > 1 and NFIGURES/NFPERROW == 1:
            ax = group_axs[i]
        elif NFIGURES == 1:
            ax = group_axs
        else:
    ...
    #fig.legend(lines, labels,fontsize=12, frameon=True)
    #ax.legend()
    if num_rows == 1 and NFIGURES> 1:
    ...
    plt.savefig('%s/group_convergence.pdf'%OUTPUTPATH, bbox_inches='tight')
...
def plot_convergence(lax=None, network=None, subfig=None):
    ...
    if network == 'lstm':
        ...
        density=1.0
        line = plot_with_params(network, 16, 20, lr, 'gpu16', r'Dense-SGD', isacc=isacc, prefix='allreduce-baseline-gwarmup-dc1-model-nips-layerwise-convlstm-adaptive', nsupdate=1, density=density, force_legend=True, force_color='r')
        plts.append(line)
        density=0.004
        #line = plot_with_params(network, 16, 20, lr, 'gpu16', r'SLGS-SGD',isacc=isacc,  prefix='allreduce-comp-topk-baseline-gwarmup-dc1-model-nips-layerwise-convlstm-adaptive', nsupdate=1, sg=2.5, density=density, force_legend=True,force_color='blue')
        ...
        plts.append(line)
        density=0.004
        #line = plot_with_params(network, 16, 20, lr, 'gpu16', r'LAGS-SGD',isacc=isacc,  prefix='allreduce-comp-topk-baseline-gwarmup-dc1-model-nips-layerwise-convlstm-adaptive', nsupdate=1, density=density, force_legend=True,force_color='g')
        line = plot_with_params(network, 16, 20, lr, 'MGD', r'LAGS-SGD',isacc=isacc,  prefix='allreduce-comp-topk-baseline-gwarmup-dc1-model-nips-layerwise-convtune-adaptive', nsupdate=1, density=density, force_legend=True,force_color='g')
        plts.append(line)
    elif network == 'resnet20':
        global_max_epochs = 120;bs=32;lr=0.1
        density=1.0
        #line = plot_with_params(network, 32, 32, lr, 'MGD', r'Dense-SGD', isacc=True, prefix='allreduce-baseline-gwarmup-dc1-model-finalconv', nsupdate=1, density=density, force_legend=True, force_color='r')
        #line = plot_with_params(network, 16, 32, lr, 'MGD', r'Dense-SGD', isacc=isacc, prefix='allreduce-baseline-gwarmup-dc1-model-layerwise-finalconv', nsupdate=1, density=density, force_legend=True, force_color='r')
        ...
        plts.append(line)
        density=0.001
        #line = plot_with_params(network, 32, bs, lr, 'MGD', r'SLGS-SGD',isacc=True,  prefix='allreduce-comp-topk-baseline-gwarmup-dc1-model-finalconv', nsupdate=1, sg=2.5, density=density, force_legend=True,force_color='blue')
        #line = plot_with_params(network, 16, 32, lr, 'MGD', r'SLGS-SGD',isacc=isacc,  prefix='allreduce-comp-topk-baseline-gwarmup-dc1-model-layerwise-finalconv', nsupdate=1, sg=2.5, density=density, force_legend=True,force_color='blue')
        ...
        plts.append(line)
        density=0.001
        #line = plot_with_params(network, 32, bs, lr, 'MGD', r'LGS-SGD',isacc=True,  prefix='allreduce-comp-topk-baseline-gwarmup-dc1-model-finalconv', nsupdate=1, density=density, force_legend=True,force_color='g')
        #line = plot_with_params(network, 16, 32, lr, 'MGD', r'LGS-SGD',isacc=isacc,  prefix='allreduce-comp-topk-baseline-gwarmup-dc1-model-layerwise-finalconv', nsupdate=1, density=density, force_legend=True,force_color='g')
        ...
        density=1.0
        #line = plot_with_params(network, 16, 128, lr, 'MGD', r'Dense-SGD', isacc=isacc, prefix='allreduce-baseline-gwarmup-dc1-model-layerwise-finalconv', nsupdate=1, density=density, force_legend=True, force_color='r')
        line = plot_with_params(network, 16, 32, lr, 'gpu16', r'Dense-SGD', isacc=isacc, prefix='allreduce-baseline-gwarmup-dc1-model-nips-layerwise-conv2-adaptive', nsupdate=1, density=density, force_legend=True, force_color='r')
        plts.append(line)
        density=0.001
        #line = plot_with_params(network, 16, 128, lr, 'MGD', r'SLGS-SGD',isacc=isacc,  prefix='allreduce-comp-topk-baseline-gwarmup-dc1-model-layerwise-finalconv', nsupdate=1, sg=2.5, density=density, force_legend=True,force_color='blue')
        ...
        plts.append(line)
        density=0.001
        #line = plot_with_params(network, 16, 128, lr, 'MGD', r'LGS-SGD',isacc=isacc,  prefix='allreduce-comp-topk-baseline-gwarmup-dc1-model-layerwise-finalconv', nsupdate=1, density=density, force_legend=True,force_color='g')
        line = plot_with_params(network, 16, 32, lr, 'gpu19', r'LAGS-SGD',isacc=isacc,  prefix='allreduce-comp-topk-baseline-gwarmup-dc1-model-nips-layerwise-conv2-adaptive', nsupdate=1, density=density, force_legend=True,force_color='g')
        plts.append(line)
    elif network == 'lstman4':
        global_max_epochs = 120;bs=32;lr=0.0002;nworkers=32
        density=1.0
        line = plot_with_params(network, 16, 4, lr, 'gpu19', r'Dense-SGD', isacc=isacc, prefix='allreduce-baseline-gwarmup-dc1-model-nips-layerwise-convlstm-adaptive', nsupdate=1, density=density, force_legend=True, force_color='r')
        plts.append(line)
        density=0.001
        ...
        global_max_epochs = 68;bs=32;lr=0.01
        #logs/allreduce-comp-topk-gwarmup-dc1-model-iclr-convergence-wu3-thres-512000kbytes
        #allreduce-comp-topk-gwarmup-dc1-model-iclr-convergence-wu3-thres-160kbytes
        density=1.0
        line = plot_with_params(network, 16, 32, lr, 'hsw224', r'Dense-SGD', isacc=isacc, prefix='allreduce-gwarmup-dc1-model-iclr-convergence-wu3-thres-512000kbytes', nsupdate=1, density=density, force_legend=True, force_color='r')
        plts.append(line)
        density=0.001
        line = plot_with_params(network, 16, 32, lr, 'scigpu10', r'SLGS-SGD',isacc=isacc,  prefix='allreduce-comp-topk-gwarmup-dc1-model-iclr-convergence-wu3-thres-512000kbytes', nsupdate=1, density=density, force_legend=True,force_color='blue')
        plts.append(line)
        #line = plot_with_params(network, 16, 32, lr, 'gpu10', r'LAGS-SGD',isacc=isacc,  prefix='allreduce-comp-topk-baseline-gwarmup-dc1-model-nips-layerwise-speedk80-adaptive', nsupdate=1, density=density, force_legend=True,force_color='g')
        line = plot_with_params(network, 16, 32, lr, 'scigpu10', r'LAGS-SGD',isacc=isacc,  prefix='allreduce-comp-topk-gwarmup-dc1-model-iclr-convergence-wu3-thres-160kbytes', nsupdate=1, density=density, force_legend=True,force_color='g')
        plts.append(line)
    elif network == 'inceptionv4':
        global_max_epochs = 20;bs=32;lr=0.01;nworkers=8
        #logs/allreduce-comp-topk-gwarmup-dc1-model-iclr-convergence-wu3-thres-512000kbytes
        #allreduce-comp-topk-gwarmup-dc1-model-iclr-convergence-wu3-thres-160kbytes
        density=1.0
        line = plot_with_params(network, nworkers, 32, lr, 'dgx08', r'Dense-SGD', isacc=isacc, prefix='allreduce-gwarmup-dc1-model-iclr-convergence-wu3-thres-16384kbytes', nsupdate=1, density=density, force_legend=True, force_color='r')
        plts.append(line)
        density=0.001
        line = plot_with_params(network, nworkers, 32, lr, 'dgx11', r'SLGS-SGD',isacc=isacc,  prefix='allreduce-comp-topk-gwarmup-dc1-model-iclr-convergence-wu3-thres-512000kbytes', nsupdate=1, density=density, force_legend=True,force_color='blue')
        #line = plot_with_params(network, 16, 64, lr, 'scigpu10', r'SLGS-SGD',isacc=isacc,  prefix='allreduce-comp-topk-gwarmup-dc1-model-iclr-convergence-wu3-thres-512000kbytes', nsupdate=1, density=density, force_legend=True,force_color='blue')
        plts.append(line)
        line = plot_with_params(network, nworkers, 32, lr, 'dgx07', r'LAGS-SGD',isacc=isacc,  prefix='allreduce-comp-topk-gwarmup-dc1-model-iclr-convergence-wu3-thres-16384kbytes', nsupdate=1, density=density, force_legend=True,force_color='g')
        plts.append(line)
    elif network == 'alexnet':
        pass
...
def read_speed(logfile):
    ...
    return avg_speed, np.mean(computations[1:]), np.mean(compressions[1:])
...
def plot_time_breakdowns():
    ...
    #networks = ['alexnet', 'resnet50', 'inceptionv4']
    num_rows = NFIGURES/NFPERROW
    for i, network in enumerate(networks):
        ax_row = i / NFPERROW
        ax_col = i % NFPERROW
...
def plot_breakdowns_with_network(ax, network):
    ...
    if dnn=='inceptionv4':
        dense_logfile = _get_log_filename('allreduce-baseline-gwarmup-dc1-model-nips-layerwise-speedfinal-adaptive', dnn=dnn, nw=nw, bs=bs, lr=lr, sparsity=None, nsupdate=1, sg=None, density=1.0, hostname='MGD')
    else:
        dense_logfile = _get_log_filename('allreduce-baseline-gwarmup-dc1-model-nips-layerwise-speed2-adaptive', dnn=dnn, nw=nw, bs=bs, lr=lr, sparsity=None, nsupdate=1, sg=None, density=1.0, hostname='MGD')
    ...
    #else:
    #    dense_logfile = _get_log_filename('allreduce-baseline-gwarmup-dc1-model-nips-layerwise-speed2-adaptive', dnn=dnn, nw=nw, bs=bs, lr=lr, sparsity=None, nsupdate=1, sg=None, density=1.0, hostname='MGD')
    #if dnn in ['vgg16', 'inceptionv4']:
    #    #topk_logfile = _get_log_filename('allreduce-comp-sigmathresallgather-baseline-gwarmup-dc1-model-nips-layerwise-speed3tune-adaptive', dnn=dnn, nw=nw, bs=bs, lr=lr, sparsity=None, nsupdate=1, sg=2.5, density=density, hostname='MGD')
    #    topk_logfile = _get_log_filename('allreduce-comp-sigmathresallgather-baseline-gwarmup-dc1-model-nips-layerwise-speedfinal-adaptive', dnn=dnn, nw=nw, bs=bs, lr=lr, sparsity=None, nsupdate=1, sg=2.5, density=density, hostname='MGD')
    #else:
    #    topk_logfile = _get_log_filename('allreduce-comp-sigmathresallgather-baseline-gwarmup-dc1-model-nips-layerwise-speed2-adaptive', dnn=dnn, nw=nw, bs=bs, lr=lr, sparsity=None, nsupdate=1, sg=2.5, density=density, hostname='MGD')
    #if dnn in ['resnet20','vgg16','lstm', 'inceptionv4']:
    #    if dnn=='lstm':
    #        lr=22.0
    #    if dnn=='inceptionv4':
    #        lw_logfile = _get_log_filename('allreduce-comp-sigmathresallgather-baseline-gwarmup-dc1-model-nips-layerwise-speedfinal-adaptive', dnn=dnn, nw=nw, bs=bs, lr=lr, sparsity=None, nsupdate=1, sg=None, density=density, hostname='MGD')
    #    else:
    #        #lw_logfile = _get_log_filename('allreduce-comp-sigmathresallgather-baseline-gwarmup-dc1-model-nips-layerwise-speed3tune-adaptive', dnn=dnn, nw=nw, bs=bs, lr=lr, sparsity=None, nsupdate=1, sg=None, density=density, hostname='MGD')
    #        lw_logfile = _get_log_filename('allreduce-comp-sigmathresallgather-baseline-gwarmup-dc1-model-nips-layerwise-speedfinal-adaptive', dnn=dnn, nw=nw, bs=bs, lr=lr, sparsity=None, nsupdate=1, sg=None, density=density, hostname='MGD')
    #else:
    #    lw_logfile = _get_log_filename('allreduce-comp-sigmathresallgather-baseline-gwarmup-dc1-model-nips-layerwise-speed2-adaptive', dnn=dnn, nw=nw, bs=bs, lr=lr, sparsity=None, nsupdate=1, sg=None, density=density, hostname='MGD')
    #dense_logfile = _get_log_filename('allreduce-baseline-gwarmup-dc1-model-nips-layerwise-speed-adjust-adaptive', dnn=dnn, nw=nw, bs=bs, lr=lr, sparsity=None, nsupdate=1, sg=None, density=1.0, hostname='MGD')
    #topk_logfile = _get_log_filename('allreduce-comp-sigmathresallgather-baseline-gwarmup-dc1-model-nips-layerwise-speed-adjust-adaptive', dnn=dnn, nw=nw, bs=bs, lr=lr, sparsity=None, nsupdate=1, sg=2.5, density=density, hostname='MGD')
    #lw_logfile = _get_log_filename('allreduce-comp-sigmathresallgather-baseline-gwarmup-dc1-model-nips-layerwise-speed-adjust-adaptive', dnn=dnn, nw=nw, bs=bs, lr=lr, sparsity=None, nsupdate=1, sg=None, density=density, hostname='MGD')
