...
#OUTPUTPATH='/media/sf_Shared_Data/tmp/icdcs2019cr'
OUTPUTPATH='/media/sf_Shared_Data/tmp/nips2019'

EPOCH = True
FONTSIZE=12

#fig, ax = plt.subplots(1,1,figsize=(5,3.8))
fig, ax = plt.subplots(1,1,figsize=(5*2,3.8*2))
ax2 = None
...
def plot_loss(logfile, label, isacc=False, title='ResNet-20'):
    ...
    if isacc and len(losses) > 0:
        print('Best val acc: %f' % np.max(losses))
...
def resnet20_convergence():
    network = 'resnet20';bs=32
    dense_name = 'DenseAllReduce'
    plot_with_params(network, 4, bs, 0.1, 'gpu18', dense_name, prefix='allreduce-baseline-gwarmup-dc1-model-nips-layerwise-nipsdebug', nsupdate=1, force_legend=True)
    layerwise_sparse = 'Adaptive Layerwise Top-k'
    plot_with_params(network, 4, bs, 0.1, 'gpu19', layerwise_sparse, prefix='allreduce-comp-topk-baseline-gwarmup-dc1-model-nips-layerwise-debug-adaptive', nsupdate=1, sg=2.5, density=0.001, force_legend=True)
    layerwise_sparse = 'Layerwise Top-k'
    plot_with_params(network, 4, bs, 0.1, 'gpu18', layerwise_sparse, prefix='allreduce-comp-topk-baseline-gwarmup-dc1-model-nips-layerwise-debug', nsupdate=1, sg=2.5, density=0.001, force_legend=True)
    layerwise_sparse = 'Top-k'
    plot_with_params(network, 4, bs, 0.1, 'gpu19', layerwise_sparse, prefix='allreduce-comp-topk-baseline-gwarmup-dc1-model-nips-debug', nsupdate=1, sg=2.5, density=0.001, force_legend=True)
    #plot_with_params(network, 4, bs, 0.1, 'gpu19', 'Sigmathres AG', prefix='allreduce-comp-sigmathresallgather-baseline-gwarmup-dc1-model-nips-debug', nsupdate=1, sg=2.5, density=0.001, force_legend=True)
    #layerwise_sparse = 'Sigma Threshold'
    #plot_with_params(network, 4, 32, 0.1, 'gpu19', layerwise_sparse, prefix='allreduce-comp-sigmathres-baseline-gwarmup-dc1-model-nips-debug', nsupdate=1, sg=2.5, density=0.001, force_legend=True)
    plt.legend(fontsize=FONTSIZE)

def vgg16_convergence():
    network = 'vgg16';bs=128
    dense_name = 'DenseAllReduce'
    plot_with_params(network, 4, bs, 0.1, 'gpu19', dense_name, prefix='allreduce-baseline-gwarmup-dc1-model-nips-layerwise-debug-adaptive', nsupdate=1, force_legend=True)
    layerwise_sparse = 'Adaptive Layerwise Top-k'
    plot_with_params(network, 4, bs, 0.1, 'gpu18', layerwise_sparse, prefix='allreduce-comp-topk-baseline-gwarmup-dc1-model-nips-layerwise-debug-adaptive', nsupdate=1, sg=2.5, density=0.001, force_legend=True)
    layerwise_sparse = 'Layerwise Top-k'
    plot_with_params(network, 4, bs, 0.1, 'gpu21', layerwise_sparse, prefix='allreduce-comp-topk-baseline-gwarmup-dc1-model-nips-layerwise-debug', nsupdate=1, sg=2.5, density=0.001, force_legend=True)
    layerwise_sparse = 'Top-k'
    plot_with_params(network, 4, bs, 0.1, 'gpu20', layerwise_sparse, prefix='allreduce-comp-topk-baseline-gwarmup-dc1-model-nips-debug', nsupdate=1, sg=2.5, density=0.001, force_legend=True)
    #layerwise_sparse = 'SigmaThres'
    #plot_with_params(network, 4, 128, 0.1, 'gpu19', layerwise_sparse, prefix='allreduce-comp-sigmathres-baseline-gwarmup-dc1-model-nips-debug', nsupdate=1, sg=2.5, density=0.001, force_legend=True)
    #layerwise_sparse = 'Layerwise SigmaThres'
    #plot_with_params(network, 4, 128, 0.1, 'gpu19', layerwise_sparse, prefix='allreduce-comp-sigmathres-baseline-gwarmup-dc1-model-nips-layerwise-debug', nsupdate=1, sg=2.5, density=0.001, force_legend=True)
    plt.legend(fontsize=FONTSIZE)
...
def plotspeed_with_params(dnn, nworkers, bs, lr, hostname, legend, isacc=False, prefix='', title='ResNet-20', sparsity=None, nsupdate=None, sg=None, density=None, force_legend=False):
    ...
    marker=markeriter.next() #STANDARD_MARKERS[algo]
    ax.plot(nws, avg_speeds, label=legend, color=fixed_colors[algo], marker=marker, linewidth=1)
...
def plot_throughputs():
    dnn='resnet20';density=0.001;bs=32;lr=0.1;nworkers=[4] #, 8, 16, 32, 64]
    ...
    dense_speeds = plotspeed_with_params(dnn=dnn, nworkers=nworkers, bs=bs, lr=lr, hostname='gpu18', legend=r'AllReduce S-SGD', prefix='allreduce-baseline-gwarmup-dc1-model-nips-layerwise-nipsdebug', title=dnn, sparsity=None, nsupdate=1, force_legend=True)
    adaptive_layerwise_topk_speeds = plotspeed_with_params(dnn=dnn, nworkers=nworkers, bs=bs, lr=lr, hostname='gpu19', legend=r'Adaptive Layerwise Top-$k$ S-SGD', prefix='allreduce-comp-topk-baseline-gwarmup-dc1-model-nips-layerwise-debug-adaptive', title=dnn, sparsity=None, nsupdate=1, sg=2.5, density=density, force_legend=True)
    topk_speeds = plotspeed_with_params(dnn=dnn, nworkers=nworkers, bs=bs, lr=lr, hostname='gpu19', legend=r'Top-$k$ S-SGD', prefix='allreduce-comp-topk-baseline-gwarmup-dc1-model-nips-debug', title=dnn, sparsity=None, nsupdate=1, sg=2.5, density=density, force_legend=True)
    layerwise_topk_speeds = plotspeed_with_params(dnn=dnn, nworkers=nworkers, bs=bs, lr=lr, hostname='gpu18', legend=r'Layerwise Top-$k$ S-SGD', prefix='allreduce-comp-topk-baseline-gwarmup-dc1-model-nips-layerwise-debug', title=dnn, sparsity=None, nsupdate=1, sg=2.5, density=density, force_legend=True)
