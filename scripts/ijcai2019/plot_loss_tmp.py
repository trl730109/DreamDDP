...
OUTPUTPATH='/media/sf_Shared_Data/tmp/icdcs2019ppt'
...
def plot_group_norm_diff():
    ...
    if NFIGURES == 6:
        ...
        plt.subplots_adjust(bottom=0.10, left=0.10, right=0.90, top=0.84, wspace=0.63, hspace=0.55)
...
def plot_norm_diff(lax=None, network=None, subfig=None):
    ...
    if network == 'lstm':
        ...
        nsupdate=16
        network = 'resnet50';bs=512;lr=0.01;epochs =40;prefix='allreduce-comp-gtopk-baseline-gwarmup-dc1-model-ijcai-norm';
    ...
    #if False and network.find('lstm') >= 0:
...
def lr_sensivities(network):
    ...
    plt.savefig('%s/%s_convergece.pdf'%(OUTPUTPATH, network))
...
def single_convergece(network):
    ...
    plt.legend(fontsize=FONTSIZE)
    plt.savefig('%s/%s_convergence.pdf'%(OUTPUTPATH, network), bbox_inches='tight')
...
def plot_group_lr_sensitivies():
    ...
    plt.subplots_adjust(bottom=0.12, left=0.11, right=0.98, top=0.95, wspace=0.31, hspace=0.49)
