...
compressors=( "none" "eftopk" "eftopkdecay" "signum" "efsignum" "efsignumdecay" )
                    ...
                    do
                        ...
                        do
                            if [ "$compressor" = "none" ]; then 
                                batch_size=$bs lr=$lr dnn=$dnn density=1.0 threshold=$thres nworkers=$nworkers compressor=$compressor momentum_correction=0 ./horovod_mpi_cj.sh
                            else
                                for density in "${densities[@]}"
                                do
                                    batch_size=$bs lr=$lr dnn=$dnn density=$density threshold=$thres nworkers=$nworkers compressor=$compressor momentum_correction=0 ./horovod_mpi_cj.sh
                                    batch_size=$bs lr=$lr dnn=$dnn density=$density threshold=$thres nworkers=$nworkers compressor=$compressor momentum_correction=1 ./horovod_mpi_cj.sh
                                done
                            fi
                        done
                    done
