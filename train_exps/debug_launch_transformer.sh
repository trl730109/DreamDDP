...
directory=$(pwd)
script="${script:-dist_debug_trainer.py}"  # Assuming this is the PyTorch distributed training script
params="${params:-}"
...
do
    ...
    args="$pre_cmd $PY -m torch.distributed.run --nproc_per_node=$ngpu_per_node --nnodes=$node_count --node_rank=$i --master_addr=$master_host --master_port=11111 $script \
        --alg $alg \
        --exp_name $exp_name \
