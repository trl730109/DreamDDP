...
# def check_dist_record(exp_tool):
#     def decoretor_check_dist_record(func):
#         def wrapper(*args, **kwargs):
#             print(f"Calling {func.__name__}")
#             if exp_tool.check_distributed_record():
#                 return func(*args, **kwargs)
#         return wrapper
#     return decoretor_check_dist_record

def check_by(method_name):
    def decorator(func):
...
class ExpTool:
    ...
    distributed = True
    only_record_root = True
    _dist = None

    @classmethod
    def check_root(cls):
        return cls._dist.get_rank() == 0

    @classmethod
    def check_distributed_record(cls):
        if cls.distributed:
            if cls.only_record_root:
                return cls.check_root()
            else:
                return True
        else:
            return True
    ...
    @classmethod
    @check_by("check_distributed_record")
    ...
    def init(cls, args, dist=None):
        ...
        if cls.check_distributed_record():
            cls.make_dir()
    ...
    @classmethod
    def init_with_sub_dir(cls, args, init_with_sub_dir, dist=None):
        cls.config = args
        cls.wandb_open = args.enable_wandb
