...
compress1 = TopKCompressor()
import torch.distributed as dist

from tuner import Tuner
# from tuner_bs_time import Tuner

comm_init()
comm = None
all_gather_comm = None
reduce_scatter_comm = None
all_to_all_comm =None
# all_to_all_comm2 =None
...
def init():
    ...
    global all_to_all_comm
    # global all_to_all_comm2
    ...
    all_to_all_comm = CommReduceScatter(op=CollectiveOp.ALL_TO_ALL)
    # all_to_all_comm2 = CommReduceScatter(op=CollectiveOp.ALL_TO_ALL)
DEBUG = False

class _DistributedOptimizer(torch.optim.Optimizer):
    def __init__(self, params, model, compression, #compression by pj
        ...
        self.exclude_alltoall = True if exclude_parts.find('alltoall') >= 0 else False
        self._compression = compression  #pj
    ...
    def _prepare_tensor_fusion(self, module_groups):
        ...
        start_p = 0
        param_groups = []
        for group_idx, module_group in enumerate(module_groups): #代码遍历每个模块组
            current_param_group = []
            start_p = 0
            for sub_idx, module in enumerate(module_group):#对于每个模块组，代码遍历其中的每个模块
                module_name = self._module_names[module] #并将其名称和对应的组索引添加到self._module_group_idx中
                self._module_group_idx[module_name] = (group_idx, sub_idx) #group_idx表示当前模块所属的模块组的索引，sub_idx表示当前模块在模块组中的索引

                for p in self._module_direct_parameters[module_name]: #遍历每个模块的直接参数，并将其名称和对应的组索引、参数组索引、参数在组内的起始位置和结束位置添加到self._param_group_idx中
                    param_name = self._param_names[p]
                    numel = p.data.numel()
                    self._param_group_idx[param_name] = (group_idx, len(current_param_group),
                            start_p, start_p+numel)
                    current_param_group.append(param_name) #将参数名称添加到当前参数组current_param_group中，并更新参数组内参数的起始位置。
                    start_p += numel

            param_groups.append(current_param_group)        
            _, pad_tensor, shard_tensor = self._get_pad_tensor(p.data, start_p, size()) # pad_tensor, shard_tensor分别存储填充后的张量和分片后的张量
            self._pad_buffers.append(pad_tensor)# self._pad_buffers所有模块组
            self._shard_buffers.append(shard_tensor)

        assert len(module_groups) == len(param_groups)
        self._num_groups = len(module_groups) #模块组数量
    ...
    @torch.no_grad()
    def _get_pad_tensor(self, tensor, numel, size): #生成填充张量和分片张量
        """
        Get padding tensors
        """
    ...
    def _register_hooks(self):
        ...
        self._indexes = {}
        self._new_name = []
        self._new_tensor_split = {}

        register_param_names = []
        for module in self._model.modules():
            params = [p for p in module.parameters(recurse=False) if p.requires_grad] #获取一个模块中所有需要进行梯度计算的参数
            direct_params = []
            for p in params: #将需要进行梯度计算的参数 params 中的每个参数 p 添加到列表 direct_params 中，并检查是否有重复的参数。
            ...
            if len(direct_params) > 0: #将需要进行梯度计算的参数 direct_params 和对应的模块 module 注册到一个列表中，以便后续的梯度计算、梯度更新等操作。
                ...
                self._module_direct_parameters[module_name] = direct_params #这个字典的作用是保存每个模块对应的需要进行梯度计算的参数列表
        ...
        for i, p in enumerate(self._register_parameters): #函数遍历 _register_parameters 列表中的每个参数 p，并为其设置梯度计算的 hook
            if self.exclude_reducescatter: # for time breakdown record
                break
            p.grad = p.data.new(p.size()).zero_() #p.grad代表参数p的梯度
            p_tmp = p.expand_as(p) #将参数 p 扩展成与自身形状相同的张量 p_tmp
            grad_acc = p_tmp.grad_fn.next_functions[0][0] #grac_acc梯度累加器 获取到梯度计算的函数
            grad_acc.register_hook(self._make_hook(p)) #在梯度计算完成后，将梯度累加到参数 p 的梯度 p.grad 中
            self._grad_accs.append(grad_acc) #将梯度累加器 grad_acc 添加到列表 _grad_accs 中
            #if rank() == 0:
    ...
    def _make_hook(self, p):
        ...
        def hook(*ignore):
            ...
            tensor = p.grad.data #tensor是梯度 
            # tensor = p.grad.data.view(-1)

            # Merging gradient tensors with padding for reduce_scatter
            new_name, pad_grad, shard_grad = self._push_to_buffer(name, tensor) #buffer满了才做rs 否则继续等 buffer_size为0 原进原出
            # 修改成A2A(topK)+accumulate by PJ (2023.09.10)
            if pad_grad is not None:
            # tensor split
                num_of_workers = size()
                n = pad_grad.size(0)
                split_size = n // num_of_workers
                # tensor_split = self.split_tensor_with_padding(pad_grad, num_of_workers)
                tensor_split = torch.split(pad_grad, split_size)
                values = []
                indexes = []
                ratio = 0.01
                for i in range(num_of_workers):
                    new_tensor_split, index, value = compress1.compress(tensor_split[i], new_name,sigma_scale=2.5,ratio = ratio, i = i, pad_grad = pad_grad )
                    values.append(value)
                ...
                # print("indexes",rank(),indexes)
                new_tensor_split = torch.zeros(tensor_split[0].numel(), device=tensor_split[0].device,
                                                  dtype=tensor_split[0].dtype)
            ...
            #     handle = reduce_scatter_comm.collective_async_(new_name, pad_grad, shard_grad)
              #  if rank() == 0:
                ...
                # if rank() == 0:
                #     if new_name == 'allToAll-group-101':
                #         print("values:",values)

                torch.cuda.synchronize()
                #All-to-ALL:
                val_results = torch.zeros(values.numel(), device=values.device, dtype=values.dtype)
                handle =  all_to_all_comm.collective_async_(new_name, values, val_results)
                idx_results = torch.zeros(indexes.numel(), device=indexes.device, dtype=indexes.dtype)
                # handle_idx = all_to_all_comm2.collective_async_(new_name, indexes, idx_results)
                handle_idx = all_to_all_comm.collective_async_(new_name, indexes, idx_results)
                # if rank() == 0:
                #     print("BP AlltoAll:", handle)

                self._values[new_name] = val_results
                self._indexes[new_name] = idx_results
                self._new_tensor_split[new_name] = new_tensor_split
                self._new_name.append(new_name) #倒序 BP
    ...
    def _push_to_buffer(self, name, tensor):
        ...
        """
        # print('tensor_size=\n', tensor.size(0))
        group_idx, sub_idx, start_p, end_p = self._param_group_idx[name] #param_group_idx #从字典中获取给定name对应的参数组索引、子索引、起始位置和结束位置
        with torch.no_grad():
            pad_buffer = self._pad_buffers[group_idx]
            pad_buffer[start_p:end_p].copy_(tensor.view(-1)) #将tensor展平成一维张量并复制到缓冲区相应位置
            self._param_group_flags[group_idx][sub_idx] = 1
            for flag in self._param_group_flags[group_idx]: #如果有任何一个标志位的值为0，说明该参数组还没有准备好进行通信，就直接返回name, None, None
                if flag == 0: # not ready
                    return name, None, None
            # comm_name = 'reduceScatter-group-%d' % group_idx # comm_name重新命名为reduceScatter-group-3(例如group_idx=3)
            comm_name = 'allToAll-group-%d' % group_idx
    ...
    def _forward_pre_hook(self, module, input):
        ...
        if torch.is_grad_enabled() and self._num_steps > 0:
            ...
            if sub_idx == 0 and self._module_group_flags[group_idx] == 0:#如果当前模块是组内的第一个模块，且该组的标志为0（即未进行过通信
                all_gather_comm.synchronize() #用于等待所有通信操作完成后再继续执行下一步操作
                self._module_group_flags[group_idx] = 1  # done
                if group_idx < self._num_groups - 1 and self._module_group_flags[group_idx+1] == 0: #如果该组不是最后一组，并且下一组的标志为0，则需要对下一组进行allgather操作，以便在下一步前向传递中使用
    ...
    def _bp_barrier(self):
        ...
        all_to_all_comm.synchronize()
        # all_to_all_comm2.synchronize()

        #测试index超出范围
        # for group_idx in range(self._num_groups):
        #     name = self._new_name[group_idx]
        #     if max(self._indexes[name]).item() > self._new_tensor_split[name].size()[0]:
        #         if rank() == 0:
        #             print("group_idx=", group_idx)
        #             print("max(self._indexes[name]=", max(self._indexes[name]))
        #             print("self._new_tensor_split[name].size()", len(self._new_tensor_split[name]))


        # if rank() == 0:
        #     name = self._new_name[5]
        #     print("self._values[name]", self._values[name])
        #     # print("name:",name)
        #     # print("self._indexes[name]", self._indexes[name])
        # if rank() == 1:
        #     name = self._new_name[5]
        #     print("self._values[name]", self._values[name])
        # if rank() == 2:
        ...
        for group_idx in range(self._num_groups):#self._num_groups
            name = self._new_name[group_idx]
            k = len(self._indexes[name]) // size()
            # if rank() == 0:
            #     print("index_name:", self._indexes[name])
            for i in range(size()): #不是k
                index = self._indexes[name][i * k:(i + 1) * k]
                # if rank() == 0:
                #     print("max_index:",self._new_tensor_split[name].size(),max(index))
                self._new_tensor_split[name][index] +=  self._values[name][i * k:(i + 1) * k]
                # self._shard_buffers.append(self._new_tensor_split[name])
            self._shard_buffers[group_idx] = self._new_tensor_split[name]

        # if rank() == 0:
        #     print("Accumulate is already")
        # if rank() == 0:
        #     name = self._new_name[5]
        #     print("self._values[name]", self._values[name])
        #     print("self._indexes[name]", self._indexes[name])
        #     print("self._new_tensor_split[name]", self._new_tensor_split[name])
        ...
        self._allgather_one_group(group_idx=0)  # 在第一个组上启动all-gather操作，以确保所有组内的设备都已经完成了reduce-scatter操作，才能进行all-gather操作
        # if rank() == 0:
...
def DistributedOptimizer(optimizer, model, compression=None, is_sparse=False, density=0.001, seq_layernames=None, layerwise_times=None, norm_clip=None, threshold=0, writer=None, gradient_path=None, fp16=False, mgwfbp=False, rdma=False, multi_job_scheduling=False, exclude_parts=''):
    ...
    return cls(optimizer.param_groups, model, compression, exclude_parts=exclude_parts)
